...
if (isset($_POST['simpan'])) {
    ...
    // Check for duplicate id_kamar or nomor_kamar
    $id_kamar = $_POST['id_kamar'];
    $nomor_kamar = $_POST['nomor_kamar'];

    $check_query = mysqli_query($koneksi, "SELECT * FROM kamar WHERE id_kamar='$id_kamar' OR nomor_kamar='$nomor_kamar'");
    
    if (mysqli_num_rows($check_query) > 0) {
        echo "<script>
                alert('Id Kamar atau Nomor Kamar sudah ada! Silakan gunakan yang lain.');
                document.location='tambahkamar.php';
              </script>";
    } else {
        if (move_uploaded_file($gambar_temp, $upload_dir . $gambar_kost)) {
            // Save data to database
            $simpan = mysqli_query($koneksi, "INSERT INTO kamar (id_kamar, nomor_kamar, harga, foto_kamar, keterangan, status) 
            VALUES ('$id_kamar', '$nomor_kamar', '$_POST[harga]', '$lokasi_foto', '$_POST[keterangan]', '$_POST[status]')");
                  ...
                  <div class="card-body">
                    <form class="forms-sample" method="POST" enctype="multipart/form-data">
                      <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="id_kamar" id="id_kamar" placeholder="Id Kamar" required>
                      </div>
                      <div class="form-group">
                        <label for="nomor_kamar">Nomor Kamar</label>
                        <input type="number" class="form-control" id="nomor_kamar" name="nomor_kamar" placeholder="Nomor Kamar" required>
                      </div>
                      <div class="form-group">
